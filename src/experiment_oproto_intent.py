...
class RunExperiment:
    ...
    def run_training_epoch(self, params, train_dl, optimizer, epoch):
        ...
        for b in train_dl:
            ...
            loss = model(x, x_len, y, y_oh, xq, xq_len, yq, yq_oh, x_ood, x_ood_len, y_ood, y_ood_oh)
        ...
        if epoch % 10 == 0:
            ...
            torch.save(state, open(os.path.join(RSL_PATH, 'oproto_intent_k100_%s.pth'%epoch), 'wb'))
        ...
        with torch.no_grad():
            ...
            for dat in dev_dl:
                ...
                for b in dat:
                    ...
                    x_str0 = [idx2word[i] for i in x_cpu[0] if i in idx2word and idx2word[i] != '</s>']
                    x_str1 = [idx2word[i] for i in x_cpu[1] if i in idx2word and idx2word[i] != '</s>']
                    x_str2 = [idx2word[i] for i in x_cpu[2] if i in idx2word and idx2word[i] != '</s>']
