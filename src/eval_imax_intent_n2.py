...
def eval_model(params, model, experiment, optimizer, epoch):
    ...
    for c in range(len(test_set)):
        ...
        save_pickle(EVAL_PATH, 'metaval_imax_intent_%s_%s.pkl'%(c,epoch), (v_macro_avg_eer, v_macro_avg_acc_ideal, v_macro_avg_acc))
        save_pickle(EVAL_PATH, 'vprobs_gts_imax_intent_%s_%s.pkl'%(c,epoch), (v_probs, v_gts))
        ...
        save_pickle(EVAL_PATH, 'metatest_imax_intent_%s_%s.pkl'%(c,epoch), (t_macro_avg_eer, t_macro_avg_acc_ideal, t_macro_avg_acc))
        save_pickle(EVAL_PATH, 'tprobs_gts_imax_intent_%s_%s.pkl'%(c,epoch), (t_probs, t_gts))
    ...
    save_pickle(EVAL_PATH, 'metaval_imax_intent_all_%s.pkl'%epoch, (v_eer_, v_acc_ideal_, v_acc_))
    save_pickle(EVAL_PATH, 'vprobs_gts_imax_intent_all_%s.pkl'%epoch, (v_probs_, v_gts_))
    ...
    save_pickle(EVAL_PATH, 'metatest_imax_intent_all_%s.pkl'%epoch, (t_eer_, t_acc_ideal_, t_acc_))
    save_pickle(EVAL_PATH, 'tprobs_gts_imax_intent_all_%s.pkl'%epoch, (t_probs_, t_gts_))
...
if __name__ == '__main__':
    ...
    for ep in [0,10,20,30,40,50,60,'best']:
        ...
        if ep =='best':
            checkpoint = torch.load(os.path.join(RSL_PATH, 'imax_intent_k100.best.pth'), map_location=lambda storage, loc: storage)
        else:
            checkpoint = torch.load(os.path.join(RSL_PATH, 'imax_intent_k100_%s.pth'%ep), map_location=lambda storage, loc: storage)
