...
class GRUEncoder(nn.Module):
    ...
    def forward(self, x_sup, kw_sup, kwidf_sup, x_sup_len, y_sup, x_q, kw_xq, kwidf_xq, x_q_len, y_q, x_neg, kw_xneg, kwidf_xneg, x_neg_len, y_neg):
        ...
        kw_sup = kw_sup.contiguous().view(1000, self.params['min_ss_size']*10, -1)
        kwidf_sup = kwidf_sup.contiguous().view(1000, self.params['min_ss_size']*10, -1)
        ...
        kwidf_sup = kwidf_sup.contiguous().view(1000, self.params['min_ss_size']*10, self.params['hidden_size'])
        ...
        x_sup_enc = x_sup_enc.contiguous().view(1000, self.params['min_ss_size'], -1)
        y_sup = y_sup.contiguous().view(1000, self.params['min_ss_size'], -1)
        
        
        
        # encoded_prototype.shape(B, Y, E) for sentence embedding of support set
        ...
        # Compute g(x, y)
        # duplicate xq to n-class labels
        x_q_enc = torch.cat((x_q_enc, x_q_enc, x_q_enc, x_q_enc, x_q_enc, x_q_enc, x_q_enc, x_q_enc, x_q_enc, x_q_enc), dim=0)
        y_q = torch.cat((y_q, y_q, y_q, y_q, y_q, y_q, y_q, y_q, y_q, y_q), dim=0)

        kw_xq_prototype = torch.cat((kw_xq_prototype, kw_xq_prototype, kw_xq_prototype, kw_xq_prototype, kw_xq_prototype, kw_xq_prototype, kw_xq_prototype, kw_xq_prototype, kw_xq_prototype, kw_xq_prototype), dim=0)

        kw_xneg_prototype = torch.cat((kw_xneg_prototype, kw_xneg_prototype, kw_xneg_prototype, kw_xneg_prototype, kw_xneg_prototype, kw_xneg_prototype, kw_xneg_prototype, kw_xneg_prototype, kw_xneg_prototype, kw_xneg_prototype), dim=0)
        ...
        labels = torch.ones(bs, 10)

        # Compute 1 - g(x, y^(\bar))
        # OOD query

        x_neg_enc = torch.cat((x_neg_enc, x_neg_enc, x_neg_enc, x_neg_enc, x_neg_enc, x_neg_enc, x_neg_enc, x_neg_enc, x_neg_enc, x_neg_enc), dim=0)
        y_neg = torch.cat((y_neg, y_neg, y_neg, y_neg, y_neg, y_neg, y_neg, y_neg, y_neg, y_neg), dim=0)
    ...
    def _predict(self, x_test_sup, x_test_sup_len, y_test_sup, x_test_q, x_test_q_len):
        ...
        x_test_sup = x_test_sup.view(1*self.params['min_ss_size'], -1)
        x_test_sup_len = x_test_sup_len.view(1*self.params['min_ss_size'])
        y_test_sup = y_test_sup.view(1*self.params['min_ss_size'], -1)
        ...
        x_sup_enc = x_sup_enc.view(100, self.params['min_ss_size'], -1)
