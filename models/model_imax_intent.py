...
class GRUEncoder(nn.Module):
    ...
    def forward(self, x_sup, x_sup_len, y_sup, x_q, x_q_len, y_q, x_neg, x_neg_len, y_neg):
        ...
        #_logits = self._compute_cos(encoded_prototype, x_neg_enc)
        ...
        logits, labels = torch.cat((logits, _logits), dim=0), torch.cat((labels, _labels), dim=0)
    ...
    def _predict(self, x_test_sup, x_test_sup_len, y_test_sup, x_test_q, x_test_q_len):
        ...
        x_sup_enc = x_sup_enc.view(1,self.params['min_ss_size'], -1)
        y_test_sup = y_test_sup.view(1,self.params['min_ss_size'], -1)
